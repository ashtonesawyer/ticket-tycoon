...
#[test]
fn to_json() {
    let upgrade1 = Upgrade::new("test-upgrade", "This is just a test", Currency::new(), Vec::new(), Effects { inc_multiplier: 0.2});
    let upgrade2 = Upgrade::new("test-upgrade", "This is just a test", Currency::new(), vec!["some-upgrade".to_string(), "some-other-upgrade".to_string()], Effects { inc_multiplier: 0.2});
    let json1 = serde_json::to_string(&upgrade1).unwrap();
    let json2 = serde_json::to_string(&upgrade2).unwrap();
    assert_eq!(json1, "{\"id\":\"test-upgrade\",\"desc\":\"This is just a test\",\"cost\":{\"cash\":0,\"xp\":0},\"requires\":[],\"effects\":{\"inc_multiplier\":0.2}}");
    assert_eq!(json2, "{\"id\":\"test-upgrade\",\"desc\":\"This is just a test\",\"cost\":{\"cash\":0,\"xp\":0},\"requires\":[\"some-upgrade\",\"some-other-upgrade\"],\"effects\":{\"inc_multiplier\":0.2}}");
}
...
impl Upgrade {
    pub fn new(id: &str, desc: &str, cost: Currency, requires: Vec<String>, effects: Effects) -> Self {
        Self {
            id: id.to_string(),
            desc: desc.to_string(),
            cost,
